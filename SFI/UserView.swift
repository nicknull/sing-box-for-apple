...
struct UserView: View {
    ...
    var body: some View {
        ...
        VStack{
            List {
                    ...
                    Section{
                        LabeledContent{
                            ...
                            HStack{
                                ...
                                Spacer()
                                Link(destination: URL(string: "mailto:LightningVPN888@gmail.com?subject=地址修复&body=请回复我最新的可访问的网址\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\(userInfo?.email ?? "火星用户")\n\(userInfo?.plan_id ?? -1 )")!) {
                                    Text("修复")
                                }
                            }
                        }
                    ...
                    .fullScreenCover(isPresented: $showScan, content: {
                        CodeScannerView(codeTypes: [.qr]) { response in
                            if case let .success(result) = response {
                                showScan = false
                                Defaults[.repair]  = result.string;
                                NewNetWorkRequest(AQAPIService.repair(address: Defaults[.host]), successCallback: {responseModel in
                                    print(responseModel)
                                })
                            }
                        }
                    ...
                    Section{
                        ForEach(products!, id:\.id) { product in
                            Button {
                                Task{
                                    do{
                                        NewNetWorkRequest(AQAPIService.getVersion(token: userManager.token), modelType: AppVersion.self) { appVersion, responseModel in
                ...
                Section {
                    ...
                    Link(destination: URL(string: "mailto:LightningVPN888@gmail.com")!, label: {
                        HStack{
                            Label {
                                Text("联系我们")
                            } icon: {
                                IVYIcon(systemName: "envelope.badge", backgroundColor: Color(hexString: "#F2DD63"))
                            }.foregroundColor(.primary)
                            Spacer()
                            Image(systemName: "chevron.forward")
                                .foregroundColor(.secondary)
                    ...
                    Link(destination: URL(string: "\(Defaults[.host])/agreement.html")!, label: {
                        HStack{
                            Label {
                                Text("用户协议")
                            } icon: {
                    ...
                    Link(destination: URL(string: "\(Defaults[.host])/protocol.html")!, label: {
                        HStack{
                            Label {
                                Text("隐私政")
                            } icon: {
                    ...
                    Link(destination: URL(string: "https://apps.apple.com/app/id1615510722?action=write-review")!, label: {
                        HStack{
                            Label {
                                Text("五星好评")
                            } icon: {
                    ...
                    Link(destination: URL(string: "https://apps.apple.com/app/id1615510722")!, label: {
                        HStack{
                            Label {
                                Text("检测更新")
                            } icon: {
        ...
        .onChange(of: auth_data, perform: { newValue in
            refresh()
        })
        ...
        .alert(isPresented: $signOut, content: {
            Alert(
                title: Text("确定登出吗?"),
